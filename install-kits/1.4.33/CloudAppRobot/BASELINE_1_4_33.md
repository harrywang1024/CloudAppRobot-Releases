# CloudApp Robot Baseline 1.4.33

This document marks the current Windows robot baseline after the
windows_db runtime-config bootstrap fix, runtime WXDB recovery hardening,
and the release-pipeline metadata sync was folded into the shipped bundle.
...
3. `windows_db` mode now auto-initializes the runtime `wechat-decrypt`
   `config.json` instead of requiring a manual copy on a fresh machine.
4. The WeChat DB reader can recover after the runtime config becomes available,
   instead of staying stuck in an import-failed state.
5. Robot status panel now shows `WXDB cfg` and `Keys` self-check status.
6. Release publishing now syncs the correct `version.json` into the runtime
   bundle so GitHub package contents no longer lag behind the published version.
7. Normal cloud runtime logging is reduced to the essential subset.
8. Diagnostic log mode is now remotely controllable from the ops side.
9. Runtime support bundles include local log-control state for investigation.
...
3. Backend `bot_version` / `runtime_bot_version` report `1.4.33`.
4. Ops console list view shows the bot version correctly.
5. `windows_db` mode starts without a manual `wechat-decrypt/config.json` copy.
6. Group-buy broadcast, order parsing, and status heartbeat all work normally.
7. Restarting the robot does not re-send already-issued stock alerts.

## Historical Note

`1.4.27` remains the earlier stabilization milestone for the resident-agent
startup fix, while `1.4.33` is now the preferred robot deployment baseline.
