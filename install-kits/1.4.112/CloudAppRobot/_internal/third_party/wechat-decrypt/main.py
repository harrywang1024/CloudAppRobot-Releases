...
def _override_runtime_config(*, db_dir, keys_file):
    """Persist caller-provided targets before find_all_keys reloads config.json."""
    from config import CONFIG_FILE, load_config

    cfg = load_config()
    changed = False

    db_dir = str(db_dir or "").strip()
    keys_file = str(keys_file or "").strip()

    if db_dir and os.path.normcase(os.path.normpath(str(cfg.get("db_dir") or ""))) != os.path.normcase(os.path.normpath(db_dir)):
        cfg["db_dir"] = db_dir
        changed = True

    if keys_file and os.path.normcase(os.path.normpath(str(cfg.get("keys_file") or ""))) != os.path.normcase(os.path.normpath(keys_file)):
    ...
    if changed:
        ...
        print(f"[*] synced extraction config: db_dir={cfg.get('db_dir')} keys_file={cfg.get('keys_file')}")
...
def ensure_keys(keys_file, db_dir):
    ...
    print()
    _override_runtime_config(db_dir=db_dir, keys_file=keys_file)
