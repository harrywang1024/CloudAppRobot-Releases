...
class ImageResolver:
    ...
    def _load_image_crypto(self):
        ...
        raw_aes = config.get("image_aes_key")
        if raw_aes:
            aes_key = str(raw_aes).strip()

        raw_xor = config.get("image_xor_key")
        if raw_xor is not None and str(raw_xor).strip() != "":
            try:
                xor_key = int(str(raw_xor).strip(), 0)
            except ValueError:
                xor_key = None

        return aes_key, xor_key

    def _get_legacy_attach_roots(self):
        roots = []
        user_profile = os.environ.get("USERPROFILE") or os.path.expanduser("~")
        account_dir = os.path.basename(os.path.normpath(self.base_dir))
        account_names = [account_dir]

        if "_" in account_dir:
            trimmed = account_dir.rsplit("_", 1)[0].strip()
            if trimmed and trimmed not in account_names:
                account_names.append(trimmed)
    ...
    def find_dat_files(self, username, file_md5):
        ...
        if os.path.isdir(search_base):
            patterns.append(os.path.join(search_base, "*", "Img", f"{file_md5}*.dat"))
        for legacy_root in self._get_legacy_attach_roots():
            patterns.append(os.path.join(legacy_root, "*", "Image", "*", f"{file_md5}*.dat"))
    ...
    def decode_image(self, username, local_id):
        ...
        aes_key, xor_key = self._load_image_crypto()
        decrypt_kwargs = {}
        if aes_key:
            decrypt_kwargs["aes_key"] = aes_key
        if xor_key is not None:
            decrypt_kwargs["xor_key"] = xor_key

        result_path, fmt = decrypt_dat_file(selected, f"{out_path_base}.tmp", **decrypt_kwargs)
        if not result_path:
