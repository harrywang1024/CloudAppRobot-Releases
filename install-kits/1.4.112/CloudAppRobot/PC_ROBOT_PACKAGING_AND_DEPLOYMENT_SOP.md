...
- Use `1.4.60` as the current stable baseline for release, manifest, and
...
## Stable Release Publish Flow

This is the recommended release order for the current stable Windows robot
line. The key point is:

- do not issue bot update commands until GitHub Release assets,
  `release-manifest.json`, and `version.json` all reflect the exact same
  version

Recommended sequence on the build machine:

1. Build the `onedir` package.
2. Run the release publisher from `robot_client`:

```powershell
powershell -ExecutionPolicy Bypass -File scripts/windows/publish_release.ps1 `
  -Version 1.4.60 `
  -Changelog "YOUR_CHANGELOG" `
  -CreateRelease
```

3. This updates local:
   - `robot_client/release-manifest.json`
   - `robot_client/version.json`
   - GitHub Release assets in `CloudAppRobot-Releases`
4. Commit and push the source repo branch that carries the new local
   `release-manifest.json` and `version.json`.
5. Sync the release repo `main` branch so its published
   `release-manifest.json` and `version.json` match the same version.
6. Verify GitHub before using Ops Console to issue an update command.

## GitHub Verification Before Issuing Update

Before pressing `Issue Update` in Ops Console, verify all of the following:

1. GitHub Release tag exists:
   - `vX.Y.Z`
2. Release assets exist:
   - `CloudAppRobot.exe`
   - `CloudAppRobot-bundle.zip`
3. `CloudAppRobot-Releases/main` contains:
   - `release-manifest.json` with stable channel version `X.Y.Z`
   - `version.json` with version `X.Y.Z`
4. The version shown in `release-manifest.json` exactly matches the target
   version you type into Ops Console.

If any one of these is still on an older version, Ops Console can correctly
fail with `HTTP 422` because the manifest does not match the requested target.

## Machine-Side Update Baseline Rules

For the current stable update chain:

1. `CloudAppAgent.exe` is the resident orchestrator.
2. `supervisor`, `runtime`, and `sender` are replaceable business roles.
3. `CloudAppUpdater.exe` is the upgrade-window helper.
4. The normal restart path after update should resume business roles only and
   must not blindly relaunch a second resident agent.

Operationally this means:

1. do not treat the agent as a normal replaceable business role
2. do not issue a new update while another update window is still active
3. if a machine is below the known-good helper baseline, first replace the
   helper files manually, then continue testing automatic updates from that
   repaired baseline

## Stable Update Validation SOP

When validating a new version against the stable baseline:

1. Start both live machines normally.
2. Confirm both machines are online and heartbeats are fresh.
3. Confirm both machines are already on the current helper baseline.
4. Publish the new GitHub Release and sync manifest/version metadata.
5. Verify GitHub metadata first.
6. Then issue update commands from Ops Console.
7. Treat `dispatched -> downloading -> installing -> succeeded` as the normal
   happy path.
8. If one machine stalls, first collect diagnostics before manual cleanup.
9. Only after diagnostics are captured should you clear stale local update
   files or manually replace helper files.

## Manual Recovery If A Machine Is Stuck

If a machine gets stuck and you need to recover it manually:

1. Stop all business-role processes on that machine.
2. Clear stale local update markers and queued command residue.
3. If needed, replace helper files with the current stable helper baseline:
   - `CloudAppUpdater.exe`
   - `_internal\CloudAppUpdater.exe`
   - if the package layout requires it, the matching helper runtime directory
4. Restart the robot normally.
5. Re-check GitHub manifest/version consistency before retrying the update.

Do not treat this recovery path as the desired steady state. The target is
still a fully automatic update from the stable helper baseline.
