    ...
    for /f "usebackq delims=" %%p in ("%RUNTIME_DIR%\group_bot_supervisor.pid") do (
        ...
        if not errorlevel 1 (
            ...
            if not errorlevel 1 (
                ...
                call "%BASE_DIR%start_robot_status_panel.cmd" "%INSTANCE_ID%"
                exit /b 0
            )
        )
    )
...
echo [NOTE] For first launch on a new PC, run this script as Administrator once so iMyna can finish local session setup.
set "CLOUDAPP_ROBOT_DATA_DIR=%DATA_ROOT%"
start "" "%EXE_PATH%" --pc-role supervisor --instance-id "%INSTANCE_ID%" --bot-config "%CONFIG_PATH%"
